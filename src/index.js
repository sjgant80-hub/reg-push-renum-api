// reg-push-renum-api · Express HTTP wrapper around reg-push-renum-sdk · MIT · AI-Native Solutions
import express from 'express';

const app = express();
app.use(express.json({ limit: '10mb' }));

app.get('/health', (_req, res) => res.json({ ok: true, tool: 'reg-push-renum', version: '1.0.0' }));

app.post('/loadRegistry', async (req, res) => {
  try {
    const { loadRegistry } = await import('@ai-native-solutions/reg-push-renum-sdk');
...
app.post('/renderChips', async (req, res) => {
  try {
    const { renderChips } = await import('@ai-native-solutions/reg-push-renum-sdk');
...
app.post('/matchesQuery', async (req, res) => {
  try {
    const { matchesQuery } = await import('@ai-native-solutions/reg-push-renum-sdk');
...
app.post('/card', async (req, res) => {
  try {
    const { card } = await import('@ai-native-solutions/reg-push-renum-sdk');
...
app.post('/escapeHtml', async (req, res) => {
  try {
    const { escapeHtml } = await import('@ai-native-solutions/reg-push-renum-sdk');
...
app.post('/escapeAttr', async (req, res) => {
  try {
    const { escapeAttr } = await import('@ai-native-solutions/reg-push-renum-sdk');
...
app.listen(PORT, () => console.log('reg-push-renum-api listening on :' + PORT));
